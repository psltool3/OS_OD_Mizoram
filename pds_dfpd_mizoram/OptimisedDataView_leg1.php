    ...
    <!-- START SCRIPTS -->
		...
		document.getElementById('downloadCSV').addEventListener('click', async function() {
			try {
				...
				const csvResponse = await fetch('api/DownloadOptimalDataOptimisedLeg1.php?format=csv&tableName=' + tableName + '&tableName1=' + tableName1 + '&district=' + district);
				const csvBlob = await csvResponse.blob();
				downloadFile(csvBlob, 'Mizoram_Optimised_Data_' + getDateString() + '.csv');
			} catch (error) {
		...
		document.getElementById('downloadXLSX').addEventListener('click', async function() {
			try {
				...
				var tableName1 = '<?php echo $tablename1 ?>';
				var district = document.getElementById('district').value;
				const excelResponse = await fetch('api/DownloadOptimalDataOptimisedLeg1.php?format=xlsx&tableName=' + tableName + '&tableName1=' + tableName1 + '&district=' + district);
				const excelBlob = await excelResponse.blob();
				downloadFile(excelBlob, 'Mizoram_Optimised_Data_' + getDateString() + '.xlsx');
			} catch (error) {
		...
		/*document.getElementById('downloadPDF').addEventListener('click', async function() {
			try {
				var tableName = '<?php echo $tablename ?>';	
				const pdfResponse = await fetch('api/DownloadOptimalDataOptimisedLeg1.php?format=pdf&tableName='+tableName);
				const pdfBlob = await pdfResponse.blob();

				const url = window.URL.createObjectURL(pdfBlob);
				const link = document.createElement('a');
				link.href = url;
				link.download = 'Mizoram_Optimised_Data_' + getDateString() + '.pdf';
				link.click();
		...
		function fetchDataFromServer(){
			...
			$.ajax({
				type: "POST",
				url: "api/fetchOptimisedDataViewLeg1.php",
				data: dataString,
				cache: false,
				error: function(){
					alert("timeout");
